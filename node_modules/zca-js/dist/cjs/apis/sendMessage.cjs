...
function sendMessageFactory(api) {
    const serviceURLs = {
        message: {
            [Message.MessageType.DirectMessage]: utils.makeURL(`${api.zpwServiceMap.chat[0]}/api/message`, {
                zpw_ver: context.appContext.API_VERSION,
                zpw_type: context.appContext.API_TYPE,
                nretry: 0,
            }),
            [Message.MessageType.GroupMessage]: utils.makeURL(`${api.zpwServiceMap.group[0]}/api/group`, {
                zpw_ver: context.appContext.API_VERSION,
                zpw_type: context.appContext.API_TYPE,
                nretry: 0,
            }),
        ...
        attachment: {
            [Message.MessageType.DirectMessage]: `${api.zpwServiceMap.file[0]}/api/message/`,
            [Message.MessageType.GroupMessage]: `${api.zpwServiceMap.file[0]}/api/group/`,
        },
    };
    const { sharefile } = context.appContext.settings.features;
    ...
    async function upthumb(filePath, url) {
        ...
        let response = await utils.request(utils.makeURL(url + "upthumb?", {
            zpw_ver: context.appContext.API_VERSION,
            zpw_type: context.appContext.API_TYPE,
    ...
    function handleMentions(type, msg, mentions) {
        ...
        const mentionsFinal = Array.isArray(mentions) && type == Message.MessageType.GroupMessage
            ? mentions
    ...
    async function handleMessage({ msg, mentions, quote }, threadId, type) {
        ...
        const isValidInstance = quote instanceof Message.Message || quote instanceof Message.GroupMessage;
        ...
        const params = quote
            ? {
                ...
                imei: isGroupMessage ? undefined : context.appContext.imei,
    ...
    async function handleAttachment({ msg, attachments, mentions, quote }, threadId, type) {
        ...
        for (const attachment of uploadAttachment) {
            ...
            const encryptedParams = utils.encodeAES(context.appContext.secretKey, JSON.stringify(data.params));
            if (!encryptedParams)
                throw new ZaloApiError.ZaloApiError("Failed to encrypt message");
        ...
        for (const gif of gifFiles) {
            ...
            const _upthumb = await upthumb(gif, serviceURLs.attachment[Message.MessageType.DirectMessage]);
     ...
     * @throws {ZaloApiError}
     */
    return async function sendMessage(message, threadId, type = Message.MessageType.DirectMessage) {
